...
context 'As a registered user and admin' do
  describe 'When I visit a station show, in addition to the visitor story' do
    before(:each) do
       ...
       @admin = create(:admin)
       create_list(:trip, 5, station: @station, start_station_id: @station.id)
       create_list(:trip, 3, end_station_id: @station.id)
    ...
    scenario 'I see the Number of rides ended at this station' do
      ...
      expect(page).to have_content("Number of Rides Ended at #{@station.name}: 3")
    end
  end
end
